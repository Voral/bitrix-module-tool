...
class ModuleListener implements \Vasoft\VersionIncrement\Contract\EventListenerInterface
{
    ...
    public function __construct(
        ...
        private readonly string $moduleId = 'vasoft.core',
        ...
        $this->projectPath = realpath(getenv('COMPOSER') ?: getcwd()) . DIRECTORY_SEPARATOR;
...
PHP;
    ...
    {
        echo __LINE__,' > ',$this->includePhpFile,PHP_EOL;
    ...
    private function writeDescription(CommitCollection $commitCollection): void
    {
        ...
        foreach ($commitCollection as $section) {
            foreach ($section->getCommits() as $commit) {
                if (in_array($commit->type, $this->excludeCommitTypes)) {
                    continue;
                }
...
PHP;
    ...
    private function getCopyDirectories(): string
    {
        ...
        if ($this->includePhpFile !== '' && file_exists($this->includePhpFile)) {
            ...
            $code = trim(preg_replace('/<\?(php)?/', '', $code));
...
PHP;
    ...
    private function getCopyAdminPages(): string
    {
        ...
        $fileList = $this->formatVarExport($files, true);
